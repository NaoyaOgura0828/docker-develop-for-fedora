# docker_develop_for_fedora
DockerでFedora開発環境を構築する。
...
2. プルダウンを`Containers`に変更し、コンテナ一覧に表示されている`docker_develop_for_fedora-fedora-1`にマウスオーバーする。
3. `docker_develop_for_fedora-fedora-1`右に表示される`フォルダアイコン`を押下する。